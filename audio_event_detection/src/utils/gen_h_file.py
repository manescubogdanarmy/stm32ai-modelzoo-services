...
def gen_h_user_file(config):
    ...
    assert config.preprocessing.target_rate in [16000, 48000], (
    ...
    if config.dataset.use_garbage_class:
        n_classes += 1
    ...
    with open(os.path.join(path, "ai_model_config.h"), "wt") as f:
        ...
        if int(config.feature_extraction.power) == 1:
            ...
            raise ValueError(f"Higher powers than 2 for spectrogram not supported in Getting Started. \n \
                             Config file had feature_extraction.power = {config.feature_extraction.power}")
        ...
        if config.feature_extraction.to_db:
            ...
            f.write('#define CTRL_X_CUBE_AI_SPECTROGRAM_SILENCE_THR   (100U) // 0 means disabled\n')
