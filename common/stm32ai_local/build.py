...
def _get_file_and_subdirectory(root_dir: Union[str, Path]):
    ...
    dirs = []
    files = []
    for item in glob.glob(os.path.join(root_dir, r'**/*'), recursive=True):  # type:Union[Path, str]
        item = Path(item)
        if item.is_dir():
            dirs.append(item)
...
def _programm_dev_board(config, series:str="", serial_number=None):
    ...
    if series == "stm32n6" and serial_number:
        ...
        if isinstance(cmd_line_network_data, list):
    ...
    if series == "stm32n6" and serial_number:
        ...
        if isinstance(cmd_line_fsbl, list):
            ...
            str_args_fsbl = config.flash_fsbl_cmd
